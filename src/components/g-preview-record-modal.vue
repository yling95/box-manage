        ...
        <div class="content">
          <div class="left">
            <div class="alarm-modal-top">
              ...
              <div class="_right" v-if="dataList[activeIndex]?.type !== 'onlyVideo'">
            ...
            >
              <g-image
                v-if="tabActive?.key === 'image' && dataList[activeIndex]?.type !== 'onlyVideo'"
                :error="defaultImage"
                :src="imageUrl"
                :scalable="true"
                :min-scale="0"
                :max-scale="50"
              />
              <div class="video-wrap" v-if="tabActive?.key === 'video'">
                <g-video-spin :spinning="dataList[activeIndex]?.spinning">
                  <video :id="`video-${dataList[activeIndex]?.id}`" class="video" autoplay muted />
                </g-video-spin>
              </div>
              <div class="video-wrap" v-if="dataList[activeIndex]?.type === 'onlyVideo'">
                <video :src="dataList[activeIndex]?.video" class="video" controls autoplay />
              </div>
            </div>
            ...
            <div class="alarm-modal-bottom">
              <div class="alarm-modal-bottom-left" v-if="!hideInfo">
                <div class="alarm-modal-bottom-info">
                  <div class="tag" v-if="dataList[activeIndex]?.tag">
                  ...
                  <div class="alarm-name" v-if="dataList[activeIndex]?.alarmName">
                    {{ dataList[activeIndex]?.alarmName }}
                  </div>
              ...
              <div class="alarm-modal-bottom-right">
                <div class="change-img-type" v-if="props.showChangeOriginalImg && originalImgUrl">
                  <a-button type="text" :class="['btn', imgType === 1 && 'btn--active']" @click="changeImgType(1)"
                    >标注图</a-button
                  >
                  <a-button type="text" :class="['btn', imgType === 0 && 'btn--active']" @click="changeImgType(0)"
                    >原图</a-button
                  >
                </div>
                <a-button
                  type="default"
                  class="icon-button"
                  :loading="openVideoLoading"
                  @click="handleDownLoad(imgType === 1 ? 'AI' : 'origin')"
                >
                  <i class="iconfont icon-download-2-line"></i>
                  {{ tabActive?.key === 'video' ? '下载视频' : '下载' }}
                </a-button>
...
export type ResourceType = 'image' | 'video' | 'onlyVideo'
...
const showListImg = () => {
  ...
  nowImgUrl.value = dataList.value[index].image
  imageUrl.value = dataList.value[index].image
...
const prev = () => {
  ...
  nextTick(() => {
    if (dataList.value[props.activeIndex].type === 'image') {
      showListImg()
    }
  })
}
...
const handleDownLoad = async (type: 'AI' | 'origin') => {
  const data = dataList.value[props.activeIndex]
  if (dataList.value[props.activeIndex]?.type === 'onlyVideo') {
    let downLink = data.downLoadLink
    let name = `${data.subTitle}-${data.time || ''}.${getFileSuffix(downLink)}`
    if (data.alarmName) {
      name = `${data.subTitle}-${data.alarmName}-${data.time || ''}.${getFileSuffix(downLink)}`
    }
    downloadFile(downLink as string, name)
    return
  }
  if (tabActive.value?.key === 'image') {
    let downLink = data.downLoadLink
    let name = `${Date.now()}-${data.tag || ''}-${data.title}${type === 'origin' ? '-A' : ''}.${getFileSuffix(
      data.image,
    )}`
    if (type === 'origin') {
      downLink = `${getFileName2(data.downLoadLink)}-A.${getFileSuffix(data.downLoadLink)}`
      name = `${Date.now()}-${data.tag || ''}-${data.title}${type === 'origin' ? '-A' : ''}.${getFileSuffix(
        data.image,
      )}`
    }
          ...
          .change-img-type {
            ...
            .btn {
              ...
              &--active {
                ...
                box-shadow: 0px 1px 3px 0px rgba(37, 37, 45, 0.14), 0px 2px 8px 0px rgba(37, 37, 45, 0.14);
              }
            }
          }
        }
