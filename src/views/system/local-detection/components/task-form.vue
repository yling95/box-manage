<template>
  <div class="task-form">
    <a-form :model="form" ref="formRef" layout="vertical" :rules="formRules">
      ...
      <a-form-item label="AI服务（多选）" name="aiSrvIdList">
        <a-select
          :loading="aiListLoading"
          v-model:value="form.aiSrvIdList"
          placeholder="AI服务"
          :options="aiList"
          allow-clear
          :showSearch="false"
...
// AI服务列表
const { loading: aiListLoading, data: aiList } = useRequest<any, any>(aiApi.getLocalDetectAiList, {
  manual: false,
  onSuccess: ({ data }) => {
...
const dataTypeOptionMap: { [key in number]: any } = {
  0: {
    ...
    info: `支持图片格式：JPG、JPEG、PNG、BMP，单张图片大小不超过5M；<br>单任务最大支持1万条检测数据，最大分辨率2560*1440`,
  ...
  1: {
    accept: 'mp4',
    info: 'MP4，单个视频大小不超过500M；单任务最大支持100条检测数据；<br>支持分辨率：352*288、640*480、1280*720、1920*1080、2560*1440',
    btnInfo: '添加数据（批量单次100个视频）',
    maxFileSize: 500.99,
    listMaxNum: 100,
...
const formRules: Record<string, Rule[]> = {
  ...
  aiSrvIdList: [{ required: true, message: '请选择AI服务', trigger: 'change' }],
