...
let currelRectificationLevel = 1
...
const autofit = {
  ...
  off(el = '#app') {
    try {
      ...
      ;(document.querySelector(currRenderDom ? currRenderDom : el)! as any).style = ''
...
function elRectification(el: string, level = 1) {
  ...
  const currEl = document.querySelectorAll(el) as any
  if (currEl.length == 0) {
...
function offelRectification() {
  ...
  for (let item of document.querySelectorAll(currelRectification) as any) {
    item.style.width = ``
    item.style.height = ``
